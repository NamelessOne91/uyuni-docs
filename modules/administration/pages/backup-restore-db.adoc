...
The backup system utilizes a dedicated, user-provided, Podman volume named [literal]``var-pgsql-walbackup`` to store:

* A base backup (a full snapshot of the database).
* WAL segments (incremental changes since the last base backup).

This volume can be created by running the following command:

[source,shell]
----
podman volume create var-pgsql-walbackup
----
...
* [literal]``var-pgsql-walbackup`` Podman volume existing on the host.
